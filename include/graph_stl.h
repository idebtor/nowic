...
//typedef pair<int, int> Pair;// for wegited graph, save for <destination, weight>
...
struct Graph {
	...
	int E;					// number of edges - unused for undirected graph, but computed 
	deque<int> *adj;		// an array of adjacency lists 
	//deque<Pair>* adj;		// an array of adjacency lists for weighted graph

	// scratch buffers that stores the graph processing results
...
void addEdge_undirected(graph g, int v, int w);		// adds two edges for undirected graph
void addEdge_directed(graph g, int v, int w);		// adds an edge for directed graph
//void addEdge_weighted(graph g, int s, int d, int w);// adds an edge for weighted graph
