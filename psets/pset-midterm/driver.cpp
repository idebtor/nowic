...
int main() {
  ...
  int shown = 32;       // show "shown" number items in the queue & items
	quecir q = newCircularQueue(min_qsize, shown);		

	do {
		cout << endl; show_setup(q); show_queue(q); show_items(q); cout << endl; 
		cout << "\te - enqueue(s)    O(1)";     cout << "\t\tr - resize(2x or 1,10,100,...)\n";
		cout << "\tE - enqueue n     O(n)";     cout << "\t\tR - reset queue\n";
    cout << "\td - dequeue       O(1)";     cout << "\t\ts - show n items\n"; 
    cout << "\tD - dequeue n     O(n)";     cout << "\t\tc - clear\n\n";  
    ...
    while (true) {
      ...
      case 'r':  
        { // a body block necessary for compiling
          if (!q->doubling) {
            q->doubling = true;
            cout << "\tQueue capacity resizes by the succssive doubling(2x). \n";
          }
          else {
            while(1) {
