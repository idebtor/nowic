 ...
 * In c, false: 0, true: any other integer except 0
 *
 * array10x([1, 2, 20], 3, 0) → true
 * array10x([3, 30], 2, 0) → true
 * array10x([3], 1, 0) → false
 * array10x([1, 2, 3, 4, 5, 6], 6, 0) → false
 * array10x([1, 2, 3, 4, 4, 50, 500, 6], 7, 0) → true
 * array10x([], 0, 0) → false
 *
 */

int array10x(int nums[], int n, int index) {
	if (n == 0 || (n - 1) == index) return 0;

	if (nums[index] * 10 != nums[index + 1])
		return array10x(nums, n, index + 1);

	return 1 || array10x(nums, n, index + 1);
}
